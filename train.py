...
# Create a simplified BFGS optimizer
def create_simple_bfgs(learning_rate=0.01, history_size=10):
    ...
    def update_fn(grads, state, params=None):
        ...
        # Use the current inverse-Hessian approximation to compute the search direction
        search_direction = -jnp.matmul(state['inv_hessian'], flat_grads)

        # Compute the step size (here simplified to a fixed learning rate)
        ...
        s = flat_params - state['prev_params']  # Parameter change
        y = flat_grads - state['prev_grad']  # Gradient change

        # Update only if this isn't the first iteration and s and y satisfy the curvature condition
        ...
        # Conditionally update the inverse Hessian
        ...
        # Reconstruct the flat update back into the parameter tree structure
        updates = []
        idx = 0
