...
def binary_operator_diag(element_i, element_j):
    a_i, bu_i = element_i
    a_j, bu_j = element_j
    return jnp.asarray(a_j * a_i, dtype=a_i.dtype), jnp.asarray(a_j * bu_i + bu_j, dtype=bu_i.dtype)
...
class LRULayer(eqx.Module):
    ...
    def __call__(self, x):
        ...
        # )
        # Ensure B_norm is explicitly complex
        B_norm = jnp.asarray(self.B_re + 1j * self.B_im, dtype=jnp.complex64) * jnp.expand_dims(
            jnp.exp(self.gamma_log), axis=-1
        )
        ...
        y = jax.vmap(lambda z, u: jnp.real(C @ z) + (self.D * u))(inner_states, x)
